...
const gridShuntedOneWay = square(
    grid.map((row, i) => rpad(lpad(row.join(''), i), row.join('').length - 1 - i).split('')),
)
const shuntedCounts = transpose(gridShuntedOneWay).map(countXmasInRow)

const gridShuntedOtherWay = square(
    grid.map((row, i) => rpad(lpad(row.join(''), row.join('').length - 1 - i), i).split('')),
)
const otherWayShuntedCounts = transpose(gridShuntedOtherWay).map(countXmasInRow)

const total =
    horizontalCounts.reduce((acc, val) => acc + val, 0) +
...
for (let r = 0; r < grid.length; r++) {
    for (let c = 0; c < grid[r].length; c++) {
        if (grid[r][c] === 'A') {
            if (r - 1 < 0 || r + 1 >= grid.length || c - 1 < 0 || c + 1 >= grid[r].length) continue
            else if (
